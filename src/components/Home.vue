<template>
  <div class="home">
    <icon name="whatsapp" scale="6"></icon>
    <p class="leading">Send a WhatsApp message to a person without adding him/her to phone book.</p>
    <b-form @submit="onSubmit">
      <b-form-select v-model="countryCode" class="mb-3">
        <option v-for="option in countryCodeOptions" v-bind:value="option.dialCode">
          {{ option.name }}
        </option>
      </b-form-select>
      <b-form-input id="phoneNumber" v-model="phoneNumber" type="tel" pattern="[0-9]*" inputmode="numeric"
                    placeholder="Enter phone number"></b-form-input>
      <br>
      <b-button :disabled="isDisabled" type="submit" variant="green-primary">
        Send Message
      </b-button>
    </b-form>
  </div>
</template>

<script>
  import 'vue-awesome/icons/whatsapp'
  import Icon from 'vue-awesome/components/Icon'

  export default {
    name: 'home',
    components: {
      Icon
    },
    computed: {
      isDisabled () {
        return this.phoneNumber === ''
      }
    },
    data () {
      return {
        countryCode: '852',
        phoneNumber: '',
        countryCodeOptions: [
          {
            'name': 'Afghanistan (+93)',
            'dialCode': '93',
            'code': 'AF'
          },
          {
            'name': 'Albania (+355)',
            'dialCode': '355',
            'code': 'AL'
          },
          {
            'name': 'Algeria (+213)',
            'dialCode': '213',
            'code': 'DZ'
          },
          {
            'name': 'American Samoa (+1 684)',
            'dialCode': '1684',
            'code': 'AS'
          },
          {
            'name': 'Andorra (+376)',
            'dialCode': '376',
            'code': 'AD'
          },
          {
            'name': 'Angola (+244)',
            'dialCode': '244',
            'code': 'AO'
          },
          {
            'name': 'Anguilla (+1 264)',
            'dialCode': '1264',
            'code': 'AI'
          },
          {
            'name': 'Antarctica (+672)',
            'dialCode': '672',
            'code': 'AQ'
          },
          {
            'name': 'Antigua and Barbuda (+1 268)',
            'dialCode': '1268',
            'code': 'AG'
          },
          {
            'name': 'Argentina (+54)',
            'dialCode': '54',
            'code': 'AR'
          },
          {
            'name': 'Armenia (+374)',
            'dialCode': '374',
            'code': 'AM'
          },
          {
            'name': 'Aruba (+297)',
            'dialCode': '297',
            'code': 'AW'
          },
          {
            'name': 'Australia (+61)',
            'dialCode': '61',
            'code': 'AU'
          },
          {
            'name': 'Austria (+43)',
            'dialCode': '43',
            'code': 'AT'
          },
          {
            'name': 'Azerbaijan (+994)',
            'dialCode': '994',
            'code': 'AZ'
          },
          {
            'name': 'Bahamas (+1 242)',
            'dialCode': '1242',
            'code': 'BS'
          },
          {
            'name': 'Bahrain (+973)',
            'dialCode': '973',
            'code': 'BH'
          },
          {
            'name': 'Bangladesh (+880)',
            'dialCode': '880',
            'code': 'BD'
          },
          {
            'name': 'Barbados (+1 246)',
            'dialCode': '1246',
            'code': 'BB'
          },
          {
            'name': 'Belarus (+375)',
            'dialCode': '375',
            'code': 'BY'
          },
          {
            'name': 'Belgium (+32)',
            'dialCode': '32',
            'code': 'BE'
          },
          {
            'name': 'Belize (+501)',
            'dialCode': '501',
            'code': 'BZ'
          },
          {
            'name': 'Benin (+229)',
            'dialCode': '229',
            'code': 'BJ'
          },
          {
            'name': 'Bermuda (+1 441)',
            'dialCode': '1441',
            'code': 'BM'
          },
          {
            'name': 'Bhutan (+975)',
            'dialCode': '975',
            'code': 'BT'
          },
          {
            'name': 'Bolivia (+591)',
            'dialCode': '591',
            'code': 'BO'
          },
          {
            'name': 'Bosnia and Herzegovina (+387)',
            'dialCode': '387',
            'code': 'BA'
          },
          {
            'name': 'Botswana (+267)',
            'dialCode': '267',
            'code': 'BW'
          },
          {
            'name': 'Brazil (+55)',
            'dialCode': '55',
            'code': 'BR'
          },
          {
            'name': 'British Indian Ocean Territory (+246)',
            'dialCode': '246',
            'code': 'IO'
          },
          {
            'name': 'Brunei Darussalam (+673)',
            'dialCode': '673',
            'code': 'BN'
          },
          {
            'name': 'Bulgaria (+359)',
            'dialCode': '359',
            'code': 'BG'
          },
          {
            'name': 'Burkina Faso (+226)',
            'dialCode': '226',
            'code': 'BF'
          },
          {
            'name': 'Burundi (+257)',
            'dialCode': '257',
            'code': 'BI'
          },
          {
            'name': 'Cambodia (+855)',
            'dialCode': '855',
            'code': 'KH'
          },
          {
            'name': 'Cameroon (+237)',
            'dialCode': '237',
            'code': 'CM'
          },
          {
            'name': 'Canada (+1)',
            'dialCode': '1',
            'code': 'CA'
          },
          {
            'name': 'Cape Verde (+238)',
            'dialCode': '238',
            'code': 'CV'
          },
          {
            'name': 'Cayman Islands (+1 345)',
            'dialCode': '1345',
            'code': 'KY'
          },
          {
            'name': 'Central African Republic (+236)',
            'dialCode': '236',
            'code': 'CF'
          },
          {
            'name': 'Chad (+235)',
            'dialCode': '235',
            'code': 'TD'
          },
          {
            'name': 'Chile (+56)',
            'dialCode': '56',
            'code': 'CL'
          },
          {
            'name': 'China, People\'s Republic of (+86)',
            'dialCode': '86',
            'code': 'CN'
          },
          {
            'name': 'Christmas Island (+61)',
            'dialCode': '61',
            'code': 'CX'
          },
          {
            'name': 'Cocos (Keeling) Islands (+61)',
            'dialCode': '61',
            'code': 'CC'
          },
          {
            'name': 'Colombia (+57)',
            'dialCode': '57',
            'code': 'CO'
          },
          {
            'name': 'Comoros (+269)',
            'dialCode': '269',
            'code': 'KM'
          },
          {
            'name': 'Congo (+242)',
            'dialCode': '242',
            'code': 'CG'
          },
          {
            'name': 'Congo, The Democratic Republic of the (+243)',
            'dialCode': '243',
            'code': 'CD'
          },
          {
            'name': 'Cook Islands (+682)',
            'dialCode': '682',
            'code': 'CK'
          },
          {
            'name': 'Costa Rica (+506)',
            'dialCode': '506',
            'code': 'CR'
          },
          {
            'name': 'Cote d\'Ivoire (+225)',
            'dialCode': '225',
            'code': 'CI'
          },
          {
            'name': 'Croatia (+385)',
            'dialCode': '385',
            'code': 'HR'
          },
          {
            'name': 'Cuba (+53)',
            'dialCode': '53',
            'code': 'CU'
          },
          {
            'name': 'Cyprus (+537)',
            'dialCode': '537',
            'code': 'CY'
          },
          {
            'name': 'Czech Republic (+420)',
            'dialCode': '420',
            'code': 'CZ'
          },
          {
            'name': 'Denmark (+45)',
            'dialCode': '45',
            'code': 'DK'
          },
          {
            'name': 'Djibouti (+253)',
            'dialCode': '253',
            'code': 'DJ'
          },
          {
            'name': 'Dominica (+1 767)',
            'dialCode': '1767',
            'code': 'DM'
          },
          {
            'name': 'Dominican Republic (+1 849)',
            'dialCode': '1849',
            'code': 'DO'
          },
          {
            'name': 'Ecuador (+593)',
            'dialCode': '593',
            'code': 'EC'
          },
          {
            'name': 'Egypt (+20)',
            'dialCode': '20',
            'code': 'EG'
          },
          {
            'name': 'El Salvador (+503)',
            'dialCode': '503',
            'code': 'SV'
          },
          {
            'name': 'Equatorial Guinea (+240)',
            'dialCode': '240',
            'code': 'GQ'
          },
          {
            'name': 'Eritrea (+291)',
            'dialCode': '291',
            'code': 'ER'
          },
          {
            'name': 'Estonia (+372)',
            'dialCode': '372',
            'code': 'EE'
          },
          {
            'name': 'Ethiopia (+251)',
            'dialCode': '251',
            'code': 'ET'
          },
          {
            'name': 'Falkland Islands (Malvinas) (+500)',
            'dialCode': '500',
            'code': 'FK'
          },
          {
            'name': 'Faroe Islands (+298)',
            'dialCode': '298',
            'code': 'FO'
          },
          {
            'name': 'Fiji (+679)',
            'dialCode': '679',
            'code': 'FJ'
          },
          {
            'name': 'Finland (+358)',
            'dialCode': '358',
            'code': 'FI'
          },
          {
            'name': 'France (+33)',
            'dialCode': '33',
            'code': 'FR'
          },
          {
            'name': 'French Guiana (+594)',
            'dialCode': '594',
            'code': 'GF'
          },
          {
            'name': 'French Polynesia (+689)',
            'dialCode': '689',
            'code': 'PF'
          },
          {
            'name': 'Gabon (+241)',
            'dialCode': '241',
            'code': 'GA'
          },
          {
            'name': 'Gambia (+220)',
            'dialCode': '220',
            'code': 'GM'
          },
          {
            'name': 'Georgia (+995)',
            'dialCode': '995',
            'code': 'GE'
          },
          {
            'name': 'Germany (+49)',
            'dialCode': '49',
            'code': 'DE'
          },
          {
            'name': 'Ghana (+233)',
            'dialCode': '233',
            'code': 'GH'
          },
          {
            'name': 'Gibraltar (+350)',
            'dialCode': '350',
            'code': 'GI'
          },
          {
            'name': 'Greece (+30)',
            'dialCode': '30',
            'code': 'GR'
          },
          {
            'name': 'Greenland (+299)',
            'dialCode': '299',
            'code': 'GL'
          },
          {
            'name': 'Grenada (+ 1 473)',
            'dialCode': '1473',
            'code': 'GD'
          },
          {
            'name': 'Guadeloupe (+590)',
            'dialCode': '590',
            'code': 'GP'
          },
          {
            'name': 'Guam (+1 671)',
            'dialCode': '1671',
            'code': 'GU'
          },
          {
            'name': 'Guatemala (+502)',
            'dialCode': '502',
            'code': 'GT'
          },
          {
            'name': 'Guernsey (+44)',
            'dialCode': '44',
            'code': 'GG'
          },
          {
            'name': 'Guinea (+224)',
            'dialCode': '224',
            'code': 'GN'
          },
          {
            'name': 'Guinea-Bissau (+245)',
            'dialCode': '245',
            'code': 'GW'
          },
          {
            'name': 'Guyana (+595)',
            'dialCode': '595',
            'code': 'GY'
          },
          {
            'name': 'Haiti (+509)',
            'dialCode': '509',
            'code': 'HT'
          },
          {
            'name': 'Holy See (Vatican City State) (+379)',
            'dialCode': '379',
            'code': 'VA'
          },
          {
            'name': 'Honduras (+504)',
            'dialCode': '504',
            'code': 'HN'
          },
          {
            'name': 'Hong Kong (+852)',
            'dialCode': '852',
            'code': 'HK'
          },
          {
            'name': 'Hungary (+36)',
            'dialCode': '36',
            'code': 'HU'
          },
          {
            'name': 'Iceland (+354)',
            'dialCode': '354',
            'code': 'IS'
          },
          {
            'name': 'India (+91)',
            'dialCode': '91',
            'code': 'IN'
          },
          {
            'name': 'Indonesia (+62)',
            'dialCode': '62',
            'code': 'ID'
          },
          {
            'name': 'Iran, Islamic Republic of (+98)',
            'dialCode': '98',
            'code': 'IR'
          },
          {
            'name': 'Iraq (+964)',
            'dialCode': '964',
            'code': 'IQ'
          },
          {
            'name': 'Ireland (+353)',
            'dialCode': '353',
            'code': 'IE'
          },
          {
            'name': 'Isle of Man (+44)',
            'dialCode': '44',
            'code': 'IM'
          },
          {
            'name': 'Israel (+972)',
            'dialCode': '972',
            'code': 'IL'
          },
          {
            'name': 'Italy (+39)',
            'dialCode': '39',
            'code': 'IT'
          },
          {
            'name': 'Jamaica (+1 876)',
            'dialCode': '1876',
            'code': 'JM'
          },
          {
            'name': 'Japan (+81)',
            'dialCode': '81',
            'code': 'JP'
          },
          {
            'name': 'Jersey (+44)',
            'dialCode': '44',
            'code': 'JE'
          },
          {
            'name': 'Jordan (+962)',
            'dialCode': '962',
            'code': 'JO'
          },
          {
            'name': 'Kazakhstan (+7)',
            'dialCode': '7',
            'code': 'KZ'
          },
          {
            'name': 'Kenya (+254)',
            'dialCode': '254',
            'code': 'KE'
          },
          {
            'name': 'Kiribati (+686)',
            'dialCode': '686',
            'code': 'KI'
          },
          {
            'name': 'Korea, Democratic People\'s Republic of (+850)',
            'dialCode': '850',
            'code': 'KP'
          },
          {
            'name': 'Korea, Republic of (+82)',
            'dialCode': '82',
            'code': 'KR'
          },
          {
            'name': 'Kuwait (+965)',
            'dialCode': '965',
            'code': 'KW'
          },
          {
            'name': 'Kyrgyzstan (+996)',
            'dialCode': '996',
            'code': 'KG'
          },
          {
            'name': 'Lao People\'s Democratic Republic (+856)',
            'dialCode': '856',
            'code': 'LA'
          },
          {
            'name': 'Latvia (+371)',
            'dialCode': '371',
            'code': 'LV'
          },
          {
            'name': 'Lebanon (+961)',
            'dialCode': '961',
            'code': 'LB'
          },
          {
            'name': 'Lesotho (+266)',
            'dialCode': '266',
            'code': 'LS'
          },
          {
            'name': 'Liberia (+231)',
            'dialCode': '231',
            'code': 'LR'
          },
          {
            'name': 'Libyan Arab Jamahiriya (+218)',
            'dialCode': '218',
            'code': 'LY'
          },
          {
            'name': 'Liechtenstein (+423)',
            'dialCode': '423',
            'code': 'LI'
          },
          {
            'name': 'Lithuania (+370)',
            'dialCode': '370',
            'code': 'LT'
          },
          {
            'name': 'Luxembourg (+352)',
            'dialCode': '352',
            'code': 'LU'
          },
          {
            'name': 'Macao (+853)',
            'dialCode': '853',
            'code': 'MO'
          },
          {
            'name': 'Macedonia, The Former Yugoslav Republic of (+389)',
            'dialCode': '389',
            'code': 'MK'
          },
          {
            'name': 'Madagascar (+261)',
            'dialCode': '261',
            'code': 'MG'
          },
          {
            'name': 'Malawi (+265)',
            'dialCode': '265',
            'code': 'MW'
          },
          {
            'name': 'Malaysia (+60)',
            'dialCode': '60',
            'code': 'MY'
          },
          {
            'name': 'Maldives (+960)',
            'dialCode': '960',
            'code': 'MV'
          },
          {
            'name': 'Mali (+223)',
            'dialCode': '223',
            'code': 'ML'
          },
          {
            'name': 'Malta (+356)',
            'dialCode': '356',
            'code': 'MT'
          },
          {
            'name': 'Marshall Islands (+692)',
            'dialCode': '692',
            'code': 'MH'
          },
          {
            'name': 'Martinique (+596)',
            'dialCode': '596',
            'code': 'MQ'
          },
          {
            'name': 'Mauritania (+222)',
            'dialCode': '222',
            'code': 'MR'
          },
          {
            'name': 'Mauritius (+230)',
            'dialCode': '230',
            'code': 'MU'
          },
          {
            'name': 'Mayotte (+262)',
            'dialCode': '262',
            'code': 'YT'
          },
          {
            'name': 'Mexico (+52)',
            'dialCode': '52',
            'code': 'MX'
          },
          {
            'name': 'Micronesia, Federated States of (+691)',
            'dialCode': '691',
            'code': 'FM'
          },
          {
            'name': 'Moldova, Republic of (+373)',
            'dialCode': '373',
            'code': 'MD'
          },
          {
            'name': 'Monaco (+377)',
            'dialCode': '377',
            'code': 'MC'
          },
          {
            'name': 'Mongolia (+976)',
            'dialCode': '976',
            'code': 'MN'
          },
          {
            'name': 'Montenegro (+382)',
            'dialCode': '382',
            'code': 'ME'
          },
          {
            'name': 'Montserrat (+1 664)',
            'dialCode': '1664',
            'code': 'MS'
          },
          {
            'name': 'Morocco (+212)',
            'dialCode': '212',
            'code': 'MA'
          },
          {
            'name': 'Mozambique (+258)',
            'dialCode': '258',
            'code': 'MZ'
          },
          {
            'name': 'Myanmar (+95)',
            'dialCode': '95',
            'code': 'MM'
          },
          {
            'name': 'Namibia (+264)',
            'dialCode': '264',
            'code': 'NA'
          },
          {
            'name': 'Nauru (+674)',
            'dialCode': '674',
            'code': 'NR'
          },
          {
            'name': 'Nepal (+977)',
            'dialCode': '977',
            'code': 'NP'
          },
          {
            'name': 'Netherlands (+31)',
            'dialCode': '31',
            'code': 'NL'
          },
          {
            'name': 'Netherlands Antilles (+599)',
            'dialCode': '599',
            'code': 'AN'
          },
          {
            'name': 'New Caledonia (+687)',
            'dialCode': '687',
            'code': 'NC'
          },
          {
            'name': 'New Zealand (+64)',
            'dialCode': '64',
            'code': 'NZ'
          },
          {
            'name': 'Nicaragua (+505)',
            'dialCode': '505',
            'code': 'NI'
          },
          {
            'name': 'Niger (+227)',
            'dialCode': '227',
            'code': 'NE'
          },
          {
            'name': 'Nigeria (+234)',
            'dialCode': '234',
            'code': 'NG'
          },
          {
            'name': 'Niue (+683)',
            'dialCode': '683',
            'code': 'NU'
          },
          {
            'name': 'Norfolk Island (+672)',
            'dialCode': '672',
            'code': 'NF'
          },
          {
            'name': 'Northern Mariana Islands (+1 670)',
            'dialCode': '1670',
            'code': 'MP'
          },
          {
            'name': 'Norway (+47)',
            'dialCode': '47',
            'code': 'NO'
          },
          {
            'name': 'Oman (+968)',
            'dialCode': '968',
            'code': 'OM'
          },
          {
            'name': 'Pakistan (+92)',
            'dialCode': '92',
            'code': 'PK'
          },
          {
            'name': 'Palau (+680)',
            'dialCode': '680',
            'code': 'PW'
          },
          {
            'name': 'Palestinian Territories, Occupied (+970)',
            'dialCode': '970',
            'code': 'PS'
          },
          {
            'name': 'Panama (+507)',
            'dialCode': '507',
            'code': 'PA'
          },
          {
            'name': 'Papua New Guinea (+675)',
            'dialCode': '675',
            'code': 'PG'
          },
          {
            'name': 'Paraguay (+595)',
            'dialCode': '595',
            'code': 'PY'
          },
          {
            'name': 'Peru (+51)',
            'dialCode': '51',
            'code': 'PE'
          },
          {
            'name': 'Philippines (+63)',
            'dialCode': '63',
            'code': 'PH'
          },
          {
            'name': 'Pitcairn (+872)',
            'dialCode': '872',
            'code': 'PN'
          },
          {
            'name': 'Poland (+48)',
            'dialCode': '48',
            'code': 'PL'
          },
          {
            'name': 'Portugal (+351)',
            'dialCode': '351',
            'code': 'PT'
          },
          {
            'name': 'Puerto Rico (+1 939)',
            'dialCode': '1939',
            'code': 'PR'
          },
          {
            'name': 'Qatar (+974)',
            'dialCode': '974',
            'code': 'QA'
          },
          {
            'name': 'Romania (+40)',
            'dialCode': '40',
            'code': 'RO'
          },
          {
            'name': 'Russia (+7)',
            'dialCode': '7',
            'code': 'RU'
          },
          {
            'name': 'Rwanda (+250)',
            'dialCode': '250',
            'code': 'RW'
          },
          {
            'name': 'Réunion (+262)',
            'dialCode': '262',
            'code': 'RE'
          },
          {
            'name': 'Saint Barthélemy (+590)',
            'dialCode': '590',
            'code': 'BL'
          },
          {
            'name': 'Saint Helena, Ascension and Tristan Da Cunha (+290)',
            'dialCode': '290',
            'code': 'SH'
          },
          {
            'name': 'Saint Kitts and Nevis (+1 869)',
            'dialCode': '1869',
            'code': 'KN'
          },
          {
            'name': 'Saint Lucia (+1 758)',
            'dialCode': '1758',
            'code': 'LC'
          },
          {
            'name': 'Saint Martin (+590)',
            'dialCode': '590',
            'code': 'MF'
          },
          {
            'name': 'Saint Pierre and Miquelon (+508)',
            'dialCode': '508',
            'code': 'PM'
          },
          {
            'name': 'Saint Vincent and the Grenadines (+1 784)',
            'dialCode': '1784',
            'code': 'VC'
          },
          {
            'name': 'Samoa (+685)',
            'dialCode': '685',
            'code': 'WS'
          },
          {
            'name': 'San Marino (+378)',
            'dialCode': '378',
            'code': 'SM'
          },
          {
            'name': 'Sao Tome and Principe (+239)',
            'dialCode': '239',
            'code': 'ST'
          },
          {
            'name': 'Saudi Arabia (+966)',
            'dialCode': '966',
            'code': 'SA'
          },
          {
            'name': 'Senegal (+221)',
            'dialCode': '221',
            'code': 'SN'
          },
          {
            'name': 'Serbia (+381)',
            'dialCode': '381',
            'code': 'RS'
          },
          {
            'name': 'Seychelles (+248)',
            'dialCode': '248',
            'code': 'SC'
          },
          {
            'name': 'Sierra Leone (+232)',
            'dialCode': '232',
            'code': 'SL'
          },
          {
            'name': 'Singapore (+65)',
            'dialCode': '65',
            'code': 'SG'
          },
          {
            'name': 'Slovakia (+421)',
            'dialCode': '421',
            'code': 'SK'
          },
          {
            'name': 'Slovenia (+386)',
            'dialCode': '386',
            'code': 'SI'
          },
          {
            'name': 'Solomon Islands (+677)',
            'dialCode': '677',
            'code': 'SB'
          },
          {
            'name': 'Somalia (+252)',
            'dialCode': '252',
            'code': 'SO'
          },
          {
            'name': 'South Africa (+27)',
            'dialCode': '27',
            'code': 'ZA'
          },
          {
            'name': 'South Georgia and the South Sandwich Islands (+500)',
            'dialCode': '500',
            'code': 'GS'
          },
          {
            'name': 'Spain (+34)',
            'dialCode': '34',
            'code': 'ES'
          },
          {
            'name': 'Sri Lanka (+94)',
            'dialCode': '94',
            'code': 'LK'
          },
          {
            'name': 'Sudan (+249)',
            'dialCode': '249',
            'code': 'SD'
          },
          {
            'name': 'Suriname (+597)',
            'dialCode': '597',
            'code': 'SR'
          },
          {
            'name': 'Svalbard and Jan Mayen (+47)',
            'dialCode': '47',
            'code': 'SJ'
          },
          {
            'name': 'Swaziland (+268)',
            'dialCode': '268',
            'code': 'SZ'
          },
          {
            'name': 'Sweden (+46)',
            'dialCode': '46',
            'code': 'SE'
          },
          {
            'name': 'Switzerland (+41)',
            'dialCode': '41',
            'code': 'CH'
          },
          {
            'name': 'Syrian Arab Republic (+963)',
            'dialCode': '963',
            'code': 'SY'
          },
          {
            'name': 'Taiwan (+886)',
            'dialCode': '886',
            'code': 'TW'
          },
          {
            'name': 'Tajikistan (+992)',
            'dialCode': '992',
            'code': 'TJ'
          },
          {
            'name': 'Tanzania, United Republic of (+255)',
            'dialCode': '255',
            'code': 'TZ'
          },
          {
            'name': 'Thailand (+66)',
            'dialCode': '66',
            'code': 'TH'
          },
          {
            'name': 'Timor-Leste (+670)',
            'dialCode': '670',
            'code': 'TL'
          },
          {
            'name': 'Togo (+228)',
            'dialCode': '228',
            'code': 'TG'
          },
          {
            'name': 'Tokelau (+690)',
            'dialCode': '690',
            'code': 'TK'
          },
          {
            'name': 'Tonga (+676)',
            'dialCode': '676',
            'code': 'TO'
          },
          {
            'name': 'Trinidad and Tobago (+1 868)',
            'dialCode': '1868',
            'code': 'TT'
          },
          {
            'name': 'Tunisia (+216)',
            'dialCode': '216',
            'code': 'TN'
          },
          {
            'name': 'Turkey (+90)',
            'dialCode': '90',
            'code': 'TR'
          },
          {
            'name': 'Turkmenistan (+993)',
            'dialCode': '993',
            'code': 'TM'
          },
          {
            'name': 'Turks and Caicos Islands (+1 649)',
            'dialCode': '1649',
            'code': 'TC'
          },
          {
            'name': 'Tuvalu (+688)',
            'dialCode': '688',
            'code': 'TV'
          },
          {
            'name': 'Uganda (+256)',
            'dialCode': '256',
            'code': 'UG'
          },
          {
            'name': 'Ukraine (+380)',
            'dialCode': '380',
            'code': 'UA'
          },
          {
            'name': 'United Arab Emirates (+971)',
            'dialCode': '971',
            'code': 'AE'
          },
          {
            'name': 'United Kingdom (+44)',
            'dialCode': '44',
            'code': 'GB'
          },
          {
            'name': 'United States (+1)',
            'dialCode': '1',
            'code': 'US'
          },
          {
            'name': 'Uruguay (+598)',
            'dialCode': '598',
            'code': 'UY'
          },
          {
            'name': 'Uzbekistan (+998)',
            'dialCode': '998',
            'code': 'UZ'
          },
          {
            'name': 'Vanuatu (+678)',
            'dialCode': '678',
            'code': 'VU'
          },
          {
            'name': 'Venezuela, Bolivarian Republic of (+58)',
            'dialCode': '58',
            'code': 'VE'
          },
          {
            'name': 'Vietnam (+84)',
            'dialCode': '84',
            'code': 'VN'
          },
          {
            'name': 'Virgin Islands, British (+1 284)',
            'dialCode': '1284',
            'code': 'VG'
          },
          {
            'name': 'Virgin Islands, U.S. (+1 340)',
            'dialCode': '1340',
            'code': 'VI'
          },
          {
            'name': 'Wallis and Futuna (+681)',
            'dialCode': '681',
            'code': 'WF'
          },
          {
            'name': 'Yemen (+967)',
            'dialCode': '967',
            'code': 'YE'
          },
          {
            'name': 'Zambia (+260)',
            'dialCode': '260',
            'code': 'ZM'
          },
          {
            'name': 'Zimbabwe (+263)',
            'dialCode': '263',
            'code': 'ZW'
          },
          {
            'name': 'Åland Islands (+358)',
            'dialCode': '358',
            'code': 'AX'
          }
        ]
      }
    },
    methods: {
      onSubmit (e) {
        e.preventDefault()
        if (this.countryCode !== '' && this.phoneNumber !== '') {
          // remove leading zero
          this.phoneNumber = this.phoneNumber.replace(/^0+/, '')
          this.$router.push({name: 'Whatsapp', params: {phoneNumber: this.countryCode + this.phoneNumber}})
        }
      }
    }
  }
</script>

<style lang="scss" scoped>
  $text: #495963;

  div {
    &.home {
      max-width: 768px;
      padding: 50px 0;
      text-align: center;
      margin: 0 auto;
      color: $text;
      p {
        &.leading {
          padding: 20px 0;
        }
      }
    }
  }
</style>
